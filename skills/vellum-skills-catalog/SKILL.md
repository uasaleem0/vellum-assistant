...
Your auto-injected Skills Catalog (pkb/skills-catalog.md, included in every conversation) lists every skill with its ID and description, split into always-on skills and load-on-demand skills. When a user asks what you can do, refer to that catalog. To use a load-on-demand skill, call skill_load with its ID first, then call its tools.
