...
async function main() {
  ...
  async function routeRequest(
    ...
    // ── Val Bridge proxy — /bridge/* -> val-bridge on localhost:3001 ──
    // RE-APPLIED 2026-09-03, after the 0.11.7 upgrade dropped it. The same
    // block existed in 0.10.0 (gateway/src/index.ts:1744) and vanished with
    // the vendored tree. That took the whole Android app down: every
    // /bridge/* call fell through to the runtime proxy's auth gate and got
    // 401, because the app sends only X-Bridge-Secret and never an
    // Authorization header. Habits/goals/workouts kept working (they hit
    // Supabase directly), so it read as a partial glitch, not an outage.
    //
    // This MUST stay ahead of the auth gate: val-bridge enforces
    // X-Bridge-Secret itself, so gateway auth in front of it is wrong rather
    // than merely redundant.
    //
    // Upgrade-fragile by nature - it lives in node_modules. The durable home
    // is an nginx `location /bridge/` block on the host, which needs root.
    // Until that exists: recorded in VAL-CUSTOM-PATCHES.md, and re-apply
    // after ANY vellum upgrade or reinstall.
