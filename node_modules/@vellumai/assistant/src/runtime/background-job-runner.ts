...
export async function runBackgroundJob(
  ...
  try {
    ...
    // VAL CUSTOM PATCH (watcher empty-prompt, re-applied 2026-09-03): when the
    // caller supplies no prompt (watcher turns), the postamble is delivered AS the
    // processMessage prompt instead of a pre-seeded message. persistUserMessage
    // rejects empty content ("Message content or attachments are required"), so
    // without this every watcher LLM dispatch fails before any model call while the
    // watcher still reports idle / 0 errors. The resulting transcript
