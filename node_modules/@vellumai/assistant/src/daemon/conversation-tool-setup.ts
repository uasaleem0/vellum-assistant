 ...
 */
// VAL CUSTOM PATCH (re-applied 2026-09-03) - see VAL-CUSTOM-PATCHES.md.
//
// Stock 0.11.7 ships ["notifications", "subagent"]. The 0.10.0 install carried
// a hand-added list of Val's core skills, and the upgrade replaced this file
// and dropped it - taking Val from 15 always-on skills to 2.
//
// The failure is silent and looks like stupidity, not breakage: nothing errors,
// Val simply stops reaching for tools she still has. Asked to list her owner's
// habits she loaded `app-builder` (the only skill named in skill_load's own
// description) and gave up, while `val-app` sat on disk unused.
//
// Preactivation costs NOTHING in prompt size: it widens ctx.allowedToolNames
// (permission) while the wire payload stays `baseDefs` - the same 16 core
// tools - see :1145-1153 below. Skill tool NAMES reach the model through
// NOW.md, which lists 58 of them; this is what makes them callable via
// skill_execute without a skill_load round trip first.
//
// Three ids from the 0.10.0 list are deliberately absent because they no
// longer exist in 0.11.7: `tasks` and `time-based-actions` (the work-items
// concept was deleted) and `browser` (no workspace skill). `ops-bus` is
// deliberately NOT here - it is load-on-demand by design.
export const DEFAULT_PREACTIVATED_SKILL_IDS = [
  "notifications",
  "subagent",
  "schedule",
