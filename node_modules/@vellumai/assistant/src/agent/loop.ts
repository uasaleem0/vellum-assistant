...
// Hard ceiling on tool-use iterations in a single turn.
//
// Re-applied 2026-09-02 after the 0.11.7 migration dropped it. Without it the
// agent loop is an unbounded `while (true)`: a self-feeding tool cycle (the
// memory-retrospective `remember` runaway, 1389 calls/hr) runs until something
// external stops it. 0.11.7 still ships no native equivalent.
//
// 150 is ~7x any legitimate turn — real turns rarely exceed ~20 iterations and
// deep autonomous runs stay well under. It bounds the pathological case only.
// Invariant: never remove this ceiling. See workspace VAL-CUSTOM-PATCHES.md §8.
  ...
  async run(options: AgentLoopRunOptions): Promise<AgentLoopRunResult> {
    ...
    while (true) {
      ...
      if (toolUseTurns >= MAX_TOOL_USE_TURNS) {
        rlog.warn(
          { turn: toolUseTurns, limit: MAX_TOOL_USE_TURNS },
          "Agent loop hit the tool-turn ceiling; stopping the turn",
        );
