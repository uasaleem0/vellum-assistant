...
export function checkBrowserRelayAuth(
  ...
  // Self-hosted default: the Chrome extension presents the capability token
  // minted by /v1/browser-extension-pair (2-part HMAC), not a gateway edge JWT.
  // The runtime already accepts this token on the upstream hop
  // (verifyHostBrowserCapability, capability-first), so authenticate it here and
  // forward its guardianId the same way the JWT path forwards the actor principal.
  const capabilityClaims = verifyHostBrowserCapability(rawToken);
  if (capabilityClaims) {
    if (capabilityClaims.expiresAt <= Date.now()) {
      log.warn(
